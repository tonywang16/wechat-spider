...
def topic_list(request):
    ...
    # 文章信息
    params = request.GET.copy()
    _obj_list = Topic.objects.order_by('-publish_time').values('id', 'wechat__id', 'wechat__name', 'wechat__status', 'avatar', 'title', 'url', 'url_perm', 'publish_time', 'available')
...
def topic_available_list(request):
    ...
    # 文章信息
    params = request.GET.copy()
    _obj_list = Topic.objects.filter(available='可用').order_by('-publish_time').values('id', 'wechat__id', 'wechat__name', 'avatar', 'title', 'url_perm', 'publish_time', 'available')
...
def wechat_topics(request, id_):
    ...
    # 文章信息
    params = request.GET.copy()
    _obj_list = Topic.objects.filter(wechat=wechat).order_by('-publish_time').values('id', 'avatar', 'title', 'url', 'url_perm', 'publish_time', 'words')
